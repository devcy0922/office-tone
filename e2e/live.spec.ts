...
test("golden path rewrite on the deployed app", async ({ page }) => {
  await page.goto("/");
  await page.getByLabel("하고 싶은 말을 그대로 적어주세요").fill(
  ...
  const result = page.locator("text=이 정도면 보내도 괜찮아요.").locator("xpath=..");
  await expect(page.getByText("이 정도면 보내도 괜찮아요.")).toBeVisible({ timeout: 45_000 });
