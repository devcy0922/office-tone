...
test("first screen is the product", async ({ page }) => {
  ...
  await expect(page.getByLabel("무슨 일이었나요?")).toBeVisible();
  await expect(page.getByLabel("하고 싶은 말은요?")).toBeVisible();
